# Programme Structure

## Overview

Our internship programs are structured to provide progressive learning experiences, building from foundational concepts to advanced implementations.

## 📅 Program Tracks

### Backend Developer Internship
- **Duration**: 4 weeks
- **Focus**: NestJS, TypeScript, PostgreSQL
- **Project**: Healthcare Management System APIs

### Full Stack Developer Internship
- **Duration**: 6 weeks
- **Focus**: Next.js, Nest.js, TypeScript
- **Project**: Schedula - Complete Application

### AI Internship Program
- **Duration**: 4 weeks
- **Focus**: AI Engineering, Code Intelligence
- **Project**: Legacy Modernization Tools
- [View AI Internship Details](../AI-Internship/README.md)

## 📦 Key Components

### [Weekly Development Plans](./Weekly-Plans.md)
Detailed breakdown of each week's objectives and deliverables

### [How We Communicate](../How-We-Communicate/00-Index.md)
Remote collaboration, Teams setup, live sessions

## 🏆 Learning Outcomes

### Technical Skills
- Modern framework proficiency
- Database design and management
- API development and testing
- Version control with Git
- Deployment and DevOps basics

### Professional Skills
- Agile methodology
- Code review processes
- Team collaboration
- Technical documentation
- Problem-solving approaches

## 📈 Progress Tracking

### Week-by-Week Goals

| Week | Backend | Full Stack | AI |
|------|---------|------------|----|
| 1 | Setup & Auth | Foundation | Fundamentals |
| 2 | Doctor APIs | Authentication | Data Processing |
| 3 | Patient APIs | Core Features | ML Models |
| 4 | Appointments | Advanced Features | Integration |
| 5 | - | Testing | - |
| 6 | - | Deployment | - |

## 📝 Assessment Methods

### Continuous Assessment
- Daily PR submissions
- Task completion quality
- Code review feedback
- Peer collaboration

### Weekly Evaluations
- Technical assessments
- Project milestones
- Presentation skills
- Problem-solving ability

### Final Assessment
- Complete project delivery
- Code quality metrics
- Documentation completeness
- Professional growth

## 🎯 Success Metrics

### Must Achieve
- ✅ 100% attendance in training sessions
- ✅ Daily PR submissions
- ✅ Core feature implementation
- ✅ Pass weekly evaluations

### Excellence Indicators
- 🌟 Exceptional code quality
- 🌟 Proactive problem-solving
- 🌟 Helping team members
- 🌟 Going beyond requirements

## 📚 Resources

### Learning Materials
- [Quick Start Guide](../Resources/Quick-Start.md)

### Tools and Platforms
- GitHub for version control
- MS Teams for communication
- Hoppscotch for API testing
- Figma for design references

---

*Last Updated: 2026-01-13*
*Navigate: [Home](../README.md) | [Handbook](../Handbook/Index.md) | [Support](../Support/Index.md) | [Teams](../Teams/Index.md)*